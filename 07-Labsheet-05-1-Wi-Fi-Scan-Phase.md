...
| :---: | :--- | :---: | :---: | :---: |
| **5.1.1** | สแกนทั่วไปทุก Channel | ESP_OK (0x0) | 18 | 2,495 ms |
| **5.1.2** | กำหนดสแกนเฉพาะ Channel 1 | ESP_OK (0x0) | 0 | 200 ms |
| **5.1.3** | กำหนดสแกน SSID ที่มีจริง (Namthip Pomhin) | ESP_OK (0x0) | 1 | 2,498 ms |
| **5.1.4** | กำหนดสแกน SSID ที่ไม่มีจริง | ESP_OK (0x0) | 0 | 2,498 ms |
...
| :---: | :--- | :--- | :---: | :---: | :--- |
| 1 | Namthip Pomhin | 16:99:36:21:B3:45 | -41 dBm | 6 | WPA2_PSK |
| 2 | <Hidden SSID> | 64:20:E3:76:88:A6 | -80 dBm | 3 | WPA2_PSK |
| 3 | สมจ๋วต🦖 | 3A:D8:5E:03:A0:25 | -80 dBm | 6 | WPA2_PSK |
| 4 | sabaiplaceI215_2.4G | 66:20:E3:16:88:A6 | -81 dBm | 3 | WPA2_PSK |
| 5 | <Hidden SSID> | 64:20:E3:80:87:26 | -82 dBm | 13 | WPA2_PSK |

<img width="604" height="329" alt="image" src="https://github.com/user-attachments/assets/50d8e57f-f07a-4655-b6c8-593f8e59b5fa" />
...
* **การลดระยะเวลา:** ช่วยลดเวลาสแกนได้อย่างมหาศาล โดยจากผลทดลองจริงลดลงจาก **2,495 ms** เหลือเพียง **200 ms** เนื่องจาก ESP32 จะส่ง Probe Request และเฝ้าฟังสัญญาณเฉพาะบน Channel 1 เพียงช่องเดียว โดยไม่ต้องเสียเวลาสลับช่องความถี่จนครบทุกช่อง (Channel 1–13)
* **ข้อจำกัด:** หาก Access Point (AP) เป้าหมายไม่ได้เปิดใช้งานอยู่บนช่องความถี่ที่ระบุ (เช่น ในการทดลอง AP `Namthip Pomhin` อยู่ที่ Channel 6) ระบบจะไม่สามารถตรวจพบ AP นั้นได้ (`ap_count = 0`)
...
* `esp_wifi_scan_start()` ส่งคืนค่า **`ESP_OK (0x0)`** เนื่องจากกระบวนการสั่งงานการสแกนระดับฮาร์ดแวร์ทำงานเสร็จสมบูรณ์
* `esp_wifi_scan_get_ap_num()` ส่งคืนค่า **`ESP_OK (0x0)`** และคืนค่าตัวแปรจำนวนเครือข่ายเป็น `ap_count = 0`
* `esp_wifi_scan_get_ap_records()` **ไม่ถูกเรียกใช้งาน** เนื่องจากโปรแกรมมีเงื่อนไขตรวจสอบ `if (ap_count == 0)` ไว้เพื่อป้องกันการขอดึงข้อมูลชุดว่าง
...
* **`-45 dBm`** แสดงถึงสัญญาณที่มีความแรงและความเสถียรมากกว่า (ผลสแกนจริงของ AP `Namthip Pomhin` วัดได้ **-41 dBm**)
* **เหตุผล:** RSSI มีหน่วยเป็น dBm เป็นสเกลแบบ Logarithmic และมีค่าเป็นลบ สัญญาณที่มีค่าตัวเลข **เข้าใกล้ 0 มากกว่า** หมายถึงกำลังส่งของสัญญาณวิทยุที่รับได้เข้มข้นกว่า จึงรับส่งข้อมูลได้ดีและเสถียรกว่าค่าที่ติดลบมากๆ เช่น -80 dBm ซึ่งเป็นสัญญาณอ่อน
...
* ค่า `authmode` เป็นสิ่งระบุประเภทความปลอดภัยและการเข้ารหัสของ AP (เช่น `WPA2_PSK` หรือ `WPA3_PSK`) 
* ESP32 จำเป็นต้องนำข้อมูลนี้ไปตั้งค่าในโครงสร้าง `wifi_config_t` ให้ตรงกับ AP ก่อนเริ่มกระบวนการเชื่อมต่อ หากกำหนดประเภทความปลอดภัยไม่ตรงกัน การทำ Authentication หรือ Handshake ในเฟสถัดไปจะล้มเหลวทันที (`REASON_AUTH_FAIL`)
